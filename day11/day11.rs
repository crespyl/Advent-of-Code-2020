...
fn occupied_los(board: &Vec<Vec<char>>, x: isize, y: isize) -> isize {
    DIRS.iter().fold(0, |sum, (dx, dy)| {
        ...
        while let Some('.') = board.get(cy as usize).and_then(|row| row.get(cx as usize)) {
            cx += dx;
            cy += dy;
        }
        if let Some('#') = board.get(cy as usize).and_then(|row| row.get(cx as usize)) {
            sum + 1
        } else {
            sum
        }
    ...
    loop {
        ...
        board.iter().enumerate().for_each(|(row, line)| {
            ...
            line.iter().enumerate().for_each(|(col, &tile)| {
                next[row][col] =
                    match (tile, occupied_neighbors(&board, col as isize, row as isize)) {
                        ('L', 0) => '#',
                        ('#', x) if x >= 4 => 'L',
